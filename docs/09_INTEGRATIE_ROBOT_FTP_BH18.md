# 9. Integratie: Wikkelrobot BH18 FTP Programma Overdracht

Dit document beschrijft de automatische overdracht van robot-wikkelprogramma's / recepten (`.PRG` / `.CNC`) naar de Wikkelrobot controller op station BH18 via FTP over het interne netwerk.

---

## 1. Scenario & Werking

1. **Order Start:** Zodra een operator een order activeert op BH18 (bijv. Order `N20025429`, Ø150mm, 16 bar), genereert het platform automatisch de CNC/robot G-code.
2. **FTP Transfer:** Een achtergrondsnoer/service maakt een FTP-verbinding met de controller van de Wikkelrobot op `192.168.10.18:21`.
3. **Recept Geheugen:** Het gegenereerde receptbestand `RECIPE_N20025429.PRG` wordt rechtstreeks geüpload naar de map `/programs/` op de robot.
4. **Machinestart:** De operator hoeft alleen de barcode op de order te scannen op het touchpanel van de robot om het reeds ingeladen programma direct te starten.

---

## 2. Volledige Broncode (`tools/integration/robot-ftp-transfer.js`)
