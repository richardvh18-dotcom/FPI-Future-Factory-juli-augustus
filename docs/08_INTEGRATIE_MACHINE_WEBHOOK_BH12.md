# 8. Integratie: Machine Webhook & Ovendeur BH12 Data

Dit document beschrijft de architectuur en code-implementatie voor automatische communicatie vanuit fabrieksmachines en PLC's naar het Future Factory platform.

---

## 1. Scenario: Ovendeur BH12 & Temperatuur Borging

Wanneer de ovendeur op uithardstation BH12 opengaat (`door_state = OPEN`):
1. Wordt het wikkel- en uithardproces van het actieve product automatisch **op gereed gemeld** (`curing_completed`).
2. Wordt de gemeten uithardtemperatuur (bijv. `145.2°C`) en de tijdsduur automatisch borgend opgeslagen in de **ISO 9001 WORM Audit Trail**.

---

## 2. HTTP POST Request Payload (Vanuit PLC / Node-RED)
...
X-Machine-API-Key: FPI_SECRET_MACHINE_KEY_2026
```
...
{
  "machineId": "BH12",
  "eventType": "OVEN_DOOR_OPENED",
  "lotNumber": "LOT123456789",
  "telemetry": {
    "temperature": 145.2,
    "curingDurationMinutes": 60
  }
}
```
...
## 3. Volledige Broncode Webhook Handler (`tools/integration/machine-webhook-handler.js`)
