...
export const fetchGeneralConfig = async () => {
  ...
  const snap = await getDoc(doc(db, ...PATHS.GENERAL_SETTINGS));
...
export const fetchMatrixConfig = async () => {
  ...
  const snap = await getDoc(doc(db, ...PATHS.MATRIX_CONFIG));
  return snap.exists() ? snap.data() : {};
};
...
export const fetchDimensions = async (pathKey) => {
  ...
  const snap = await getDocs(collection(db, ...PATHS[pathKey]));
...
export const fetchLabelTemplates = async () => {
  ...
  const snap = await getDocs(collection(db, ...PATHS.LABEL_TEMPLATES));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
};
...
export const fetchPrinters = async () => {
  ...
  const snap = await getDocs(collection(db, ...PATHS.PRINTERS));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
};
