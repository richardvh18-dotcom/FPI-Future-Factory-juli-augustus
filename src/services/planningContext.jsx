...
export const getRawPlanningData = async (limitCount = 50) => {
  try {
    const paths = [
      ...
      PATHS?.PLANNING_LEGACY || ["future-factory", "production", "data", "digital_planning", "orders"],
    ];

    const snapshots = await Promise.all(
      paths.map((p) =>
        getDocs(query(collection(db, ...(Array.isArray(p) ? p : [p])), limit(limitCount)))
          .catch(() => ({ docs: [] }))
      )
    );
...
export const getTodayProductionContext = async () => {
  try {
    ...
    const trackSnap = await getDocs(
      query(
        collection(db, ...(Array.isArray(trackingPath) ? trackingPath : [trackingPath])),
        limit(800)
      )
    ).catch(() => ({ docs: [] }));
    ...
    const archSnap = await getDocs(
      query(collection(db, ...archivePath), limit(500))
    ).catch(() => ({ docs: [] }));
