 ...
 */
const Header = ({ searchQuery, setSearchQuery, onSearchSubmit, isSearching, logoUrl, appName, onMenuToggle }: HeaderProps) => {
    ...
    <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-3 md:px-6 shrink-0 z-50 shadow-lg fixed top-0 left-0 right-0 md:relative">
      ...
      <div className="flex items-center gap-2 md:gap-4 md:min-w-[280px]">
        <div className="flex items-center gap-2 md:gap-3 cursor-pointer group" onClick={() => navigate("/")}>
          {logoUrl ? (
            <img src={logoUrl} alt={appName || "Logo"} className="h-8 w-8 md:h-10 md:w-10 object-contain" />
          ) : (
      ...
      <div className="flex-1 max-w-2xl px-2 md:px-4">
        <div className="relative group">
          ...
          <div className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="p-1 hover:bg-white/10 rounded-lg text-slate-500 transition-colors">
                <X size={14} />
              </button>
            )}
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${
                isAIMode || searchQuery.startsWith('?') ? 'bg-purple-500 text-white' : 'hover:bg-white/10 text-slate-500'
              }`}
