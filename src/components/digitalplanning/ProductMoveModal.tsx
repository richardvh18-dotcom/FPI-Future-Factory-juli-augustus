...
interface ProductMoveModalProps {
  product: Product;
  onClose: () => void;
  onMove: (lotNumber: string, stationName: string) => void;
  allowedStations?: Station[];
  currentDepartment?: string;
}

const ProductMoveModal: FC<ProductMoveModalProps> = ({
  product,
  onClose,
  onMove,
  allowedStations = [],
  ...
  const [stationToConfirm, setStationToConfirm] = useState<string | null>(null);
  ...
  const otherDepartments = useMemo(() => {
    ...
    return departments.filter((d: Department) => d.id.toLowerCase() !== currentDepartment.toLowerCase());
  }, [currentDepartment]);

  const handleStationClick = (stationName: string): void => {
      ...
      <div className="bg-white rounded-[24px] sm:rounded-[30px] shadow-2xl w-full max-w-2xl p-5 sm:p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            {otherDepartments.map((dept: Department) => (
              <button
                key={dept.id}
