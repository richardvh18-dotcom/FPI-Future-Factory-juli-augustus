 ...
 */
export const useTerminalData = ({ initialStation, orders = [], planningSearch: initialPlanningSearch, wikkelenSearch: initialWikkelenSearch, lossenPlanningFilter: initialLossenPlanningFilter, referenceDate: initialReferenceDate, showAllWeeks: initialShowAllWeeks }: any) => {
  ...
  const [planningSearch, setPlanningSearch] = useState(initialPlanningSearch || "");
  const [wikkelenSearch, setWikkelenSearch] = useState(initialWikkelenSearch || "");
  ...
  const [referenceDate, setReferenceDate] = useState<Date>(initialReferenceDate || new Date());
  const [showAllWeeks, setShowAllWeeks] = useState(initialShowAllWeeks ?? true); // STANDAARD AAN: Toon alle weken met weekdividers
  ...
  const parseDateSafe = (dateInput: unknown) => {
    return toDateSafe(dateInput as never);
  };
  ...
  const handleViewDrawing = async (productId: string | TrackedProductDoc) => {
    ...
    notify((String(t("digitalplanning.terminal.product_not_found"))) as never);
    } catch (err) {
  ...
  const handleStartProduction = async (
    ...
    try {
      ...
      const printerId = String((startOptions as Record<string, unknown>)?.printerId || "").trim();
      const skipStartLabel = Boolean((startOptions as Record<string, unknown>)?.skipStartLabel);
      const requestedLabelCount = Math.max(
        1,
        Number.parseInt(String((startOptions as Record<string, unknown>)?.requestedLabelCount || "1"), 10) || 1
      );
