...
export const useTeamleaderModalData = ({
	...
	return useMemo(() => {
		...
		const getItemDateForKpi = (item) => {
			const candidates =
				activeKpi === "gereed"
					? [item?.timestamps?.finished, item?.timestamps?.completed, item?.updatedAt, item?.lastUpdated, item?.createdAt]
					: [item?.inspection?.timestamp, item?.timestamps?.rejected, item?.archivedAt, item?.updatedAt, item?.lastUpdated, item?.createdAt];
