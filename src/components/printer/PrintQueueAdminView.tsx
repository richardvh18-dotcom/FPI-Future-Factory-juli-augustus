...
const normalizeStationBindingKey = normalizeQueueStationKey;
...
const TempLabelModal = ({ onClose, labelTemplates = [], labelRules = [], printerDpi = 203, usbDevice, setUsbDevice, activeQueuePrinter, requestLabelsQueuePrinter, selectedStation, departmentGroups = [], printers = [] }: TempLabelModalProps) => {
  ...
  const selectedOrder = useMemo(() => {
    ...
    return displayItems.find((item) => String(item.id || "") === selectedOrderId)
      || machineItems.find((item) => String(item.id || "") === selectedOrderId)
      || null;
...
import { usePrintQueueAdmin } from './usePrintQueueAdmin';
