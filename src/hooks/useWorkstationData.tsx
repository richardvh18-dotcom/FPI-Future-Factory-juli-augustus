...
import ActiveProductionView from "../components/digitalplanning/views/ActiveProductionView";
...
import { WorkstationModals } from "../components/digitalplanning/WorkstationModals";

import Terminal from "../components/digitalplanning/Terminal";
import Nabewerken from "../components/digitalplanning/Nabewerken";
import LossenView from "../components/digitalplanning/LossenView";
import MazakView from "../components/digitalplanning/MazakView";
import GereedView from "../components/digitalplanning/GereedView";
import BM01Hub from "../components/digitalplanning/BM01Hub";
...
const getLossenRoute = (itemText: unknown, originStation = "") => {
  ...
  if (originNorm === "BH31" || originNorm === "BH16") return {
    mode: "STATION", station: "LOSSEN" };
...
export const useWorkstationData = () => {
  ...
  const handleOperatorShiftCheckin = async (badgeOverride?: unknown) => {
    ...
    try {
      ...
      let person: PersonnelEntry | undefined = personnel.find((p: PersonnelEntry) => {
  ...
  const handleStartProduction = async (
    ...
    try {
      ...
      const startResult = (await startWorkstationProductionRun({
        ...
        stationOperators: stationOperators as string[],
        source: "WorkstationHub",
  ...
  const handleProcessUnit = async (product: TrackedProductDoc, options: { bulkUnits?: TrackedProductDoc[] } = {}) => {
    ...
    try {
      ...
      const routingResult = (await routeTrackedProductsToLossen({
        ...
        centralOperators: lossenOperators as string[],
        actorLabel: currentUser?.email || "Operator",
        source: "WorkstationHub",
