  ...
  try {
    ...
    if (EventStore && EventStore.append) {
      await EventStore.append({
        type: eventType === 'LOT_TRANSITIONED' ? 'OrderTransferred' : 'OrderTransferred', 
...
const moveTrackedProductManualService = async ({
  ...
  await IdempotencyRegistry.checkAndLock(commandId, { action: 'moveTrackedProductManualService' });
